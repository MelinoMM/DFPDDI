...
def load_data(args, val_ratio=0.1, test_ratio=0.2):
    ...
    def loadtrainvaltest():
        ...
        train_pos = [(h, t, r) for h, t, r in zip(
            train['d1'], train['d2'], train['type'])]
        ...
        val_pos = [(h, t, r)
                   for h, t, r in zip(val['d1'], val['d2'], val['type'])]
        # np.random.seed(args.seed)
        ...
        test_pos = [(h, t, r)
                    for h, t, r in zip(test['d1'], test['d2'], test['type'])]
        # np.random.seed(args.seed)
    ...
    features_o = normalize(features)  # original features
    ...
    data_o = Data(x=x_o, edge_index=edge_index_o.t(
    ).contiguous(), edge_type=label_list)
    
    # adversarial nodes
    x_a = torch.tensor(features_a, dtype=torch.float)
    data_s = Data(x=x_a, edge_index=edge_index_o.t(
    ).contiguous(), edge_type=label_list)
